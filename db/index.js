...
const DATABASE_URL = "postgres://localhost:5432/phenomena-dev";
...
async function getOpenReports() {
  try {
    ...
    const { rows: openReport } = await client.query(
      `
      SELECT * FROM reports WHERE ("expirationDate" > CURRENT_TIMESTAMP);
      `
    );
    console.log(openReport);
    let now = new Date();
    let returnArray = [];
    //if (!Array.isArray(openReport)) {
    //returnArray.push(openReport);
    //} else {
    //returnArray = openReport;
    //}
    returnArray = openReport.map(function (post) {
      let thisPost = post;
      if (thisPost.expirationDate > now) {
        thisPost.isExpired = false;
      } else {
        thisPost.isExpired = true;
      }
      return thisPost;
    });

    reportIds = returnArray.map(function (report) {
      return report.id;
    });
    ...
    const {
      ...
      `
      SELECT * FROM comments WHERE "reportId" IN ($1);
      `,
      reportIds
    );
    ...
    if (!Array.isArray(comments)) {
      returnComments.push(comments);
    } else {
      returnComments = comments;
    }

    for (let report of returnArray) {
      deezComments = returnComments.map(function (comment) {
        if (comment.reportId == report.id) {
          return comment;
        }
      });
      console.table(deezComments);
      report.comments = deezComments;
    }
...
async function createReport(reportFields) {
  ...
  try {
    ...
    const {
      ...
      `
      INSERT INTO reports(title, location, description, password) VALUES ($1, $2, $3, $4) RETURNING id, title, location, description, "isOpen", "expirationDate";
      `,
